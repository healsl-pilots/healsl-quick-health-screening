...
The open-ended Final Feedback is listed as-is (every response, with its area) in the app
Analysis view and on the Sheet Dashboard. No external service is used, so the script stays
within the spreadsheet-only permission that Google Advanced Protection allows.
